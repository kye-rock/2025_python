...
l_frame = Frame(root, padx=8, pady=8)
l_frame.pack(side="left", fill="both", expand=True)

r_frame = Frame(root, padx=8, pady=8)
r_frame.pack(side="right", fill="both", expand=True)

Label(l_frame, text="메뉴목록", justify=LEFT, padx=20).pack()
Label(r_frame, text="장바구니", justify=LEFT, padx=20).pack()

# justify 옵션 삭제
menu_list = Listbox(l_frame, height=10)
...
Label(r_frame, text="장바구니").pack(anchor="w")
cart_list = Listbox(r_frame, height=10)
cart_list.pack(fill="both", expand=True)

bottom = Frame(r_frame)
