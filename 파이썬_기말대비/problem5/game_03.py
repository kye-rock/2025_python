    ...
    def hit_paddle(self, pos):
        ...
        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2]:
            if pos[3] >= paddle_pos[1] and pos[3] <= paddle_pos[3]:
                ...
                # 추가 1) 패들에 닿을 때마다 공의 색깔이 바뀌는 코드
                colors = ["red", "orange", "yellow", "green", "blue", "purple"]
                self.canvas.itemconfig(self.id, fill = random.choice(colors))
                return True
        return False
...
def run_game():
    ...
    while True:
        if ball.hit_bottom == False:
            ...
            # 추가 2) 15점이 되면 게임에서 승리하는 코드
            if ball.score == 3:
                canvas.create_text(250, 180, text = "YOU WIN", font = ("Helvetica", 30), fill = "red")
                canvas.create_text(250, 220, text = "점수: " + str(ball.score), font = 15)
            ...
            canvas.create_text(250, 180, text = "GAME OVER", font = ("Helvetica", 30), fill = "red")
            canvas.create_text(250, 220, text = "점수: " + str(ball.score), font = 15)
